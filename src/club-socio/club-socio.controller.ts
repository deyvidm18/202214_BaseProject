...
export class ClubSocioController {
    ...
    async addMemberToClub(
        ...
        return await this.clubSocioService
            .addMemberToClub(clubId, socioId);
    }
    ...
    async findMemberFromClub(
        ...
        @Param('socioId') socioId: string) {
        return await this.clubSocioService
            .findMemberFromClub(clubId, socioId);
    }

    @Put(':clubId/members')
    async updateMembersFromClub(
        @Body() socisoDto: SocioDto[],
        @Param('clubId') clubId: string) {
        const socios = plainToInstance(SocioEntity, socisoDto);
        return await this.clubSocioService
            .updateMembersFromClub(clubId, socios);
    }
    ...
    async deleteMemberFromClub(
        ...
        @Param('socioId') socioId: string) {
        return await this.clubSocioService
            .deleteMemberFromClub(clubId, socioId);
    }
